...
- [x] Asignación de pedido → push FCM al repartidor con sonido/prioridad alta — `fcm_service.send_push` construido e integrado en `assign_driver`, con `AndroidConfig(priority="high", ...sound="default")`; **no probado contra un proyecto Firebase real**, porque todavía no existe uno (pendiente de que se cree y se entreguen credenciales) — se degrada a no-op con warning único mientras `fcm_credentials_json` no esté configurado, probado explícitamente en ese modo y en los modos configurado-éxito/configurado-falla vía mocking de `firebase_admin.messaging.send`
- [x] WebSocket `/ws/driver/{token}`: recibe pings GPS → persiste en `location_pings` cada 60s — **Redis reemplazado por un `ConnectionManager` en proceso** (decisión de arquitectura flagged y aprobada explícitamente; ver `docs/digital-debt.md` — un solo backend, sin escalamiento horizontal todavía, Redis sigue disponible vía `docker-compose`/dependencia pero sin cablear); cada ping actualiza la posición en memoria y transmite de inmediato, la escritura en `location_pings` se throttlea a 60s por repartidor; probado con un cliente WebSocket sintético (`httpx-ws`), porque la app Flutter del repartidor todavía no existe
- [x] WebSocket `/ws/dispatch/{token}`: emite eventos del canal del negocio (posiciones + cambios de pedido) — mismo `ConnectionManager`, sin Redis; emite `driver_position`, `positions_snapshot` (al conectar, hidrata con las posiciones ya conocidas), `order_created` y `order_status_changed`; autenticación vía el access token JWT existente como segmento de la URL (mismo patrón que `tracking_token`/enlaces de invitación), no un token nuevo
...
- [x] Mapa en vivo (Leaflet): repartidores con color por estado, pedidos activos, actualización por WebSocket — `MapaPage` consume `/ws/dispatch/{token}` (repartidores por `DriverStatus`: offline/online/busy, coloreados; posición inicial desde `GET /drivers`'s `last_lat/last_lng`, live-actualizada por WebSocket con reconexión automática); usa la librería `leaflet` directa (imperativa), no `react-leaflet` (no es una dependencia del proyecto); **"pedidos activos" simplificado**: reusa `GET /orders?on_date=hoy` (mismo patrón que `PedidosPage`) filtrando en el cliente los estados no-terminales, en vez de un endpoint dedicado — simplificación deliberada a la escala actual, documentada en `docs/digital-debt.md` para revisitar si el volumen diario de pedidos lo justifica. **Esta línea es solo el mapa en vivo del panel interno** — el mapa en vivo de la página pública de tracking es explícitamente Fase 2 (línea 268: "mapa con repartidor en vivo"), fuera de alcance aquí, sin tocar en este checkpoint
...
**App Flutter:**
- [x] Login (email/contraseña) + registro de FCM token — login construido con Riverpod (auth state, dio con interceptor de refresh-on-401 espejo de `dispatch-web/src/api/client.ts`), `flutter_secure_storage` para el par de tokens, y `ServerConfigScreen` (URL del backend configurable en runtime, no baked-in — necesario porque la IP LAN de esta máquina cambia entre reconexiones); **probado en un dispositivo real (Redmi 10 5G)**, incluyendo un bug real encontrado y resuelto en ese mismo proceso (`https://` aceptado por error en vez de solo `http://`, causando un fallo silencioso — ver `docs/digital-debt.md`). **Registro de FCM token: solo la mitad construida.** `PATCH /api/v1/auth/me/fcm-token` (backend) y `AuthRepository.registerFcmToken()` (Flutter) existen, están probados contra el endpoint real, y compilan — pero no tienen ningún llamador todavía: no existe un proyecto Firebase (`google-services.json`) para que `firebase_messaging` obtenga un token real del dispositivo, así que ese paquete deliberadamente no está agregado aún (agregarlo sin el JSON real rompe el build). Pendiente: crear el proyecto Firebase, agregar `firebase_core`/`firebase_messaging`, y conectar la llamada real al login.
